/*
            SAMPLE ASSESSMENT2_4
*/

public class Assess2_4_Sample{

  public static String checkCenter(int width, int height, int tile1){
    int tileRow = (tile1 - 1) / width;
    int tileCol = (tile1 - 1) % width;

    
    int minCenterRow = (height - 1) / 2;
    int maxCenterRow = height / 2;
    int minCenterCol = (width  - 1) / 2;
    int maxCenterCol = width  / 2;

    if (tileRow >= minCenterRow && tileRow <= maxCenterRow &&
        tileCol >= minCenterCol && tileCol <= maxCenterCol) {
        return "center";
    }
    return "not center";
  }
