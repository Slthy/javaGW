...
public class Assess2_1_Sample{
  ...
  public static int findQuadrant(int width, int height, int tile){
    ...
    int topRows  = (height + 1) / 2;
    int leftCols = (width  + 1) / 2;

    boolean bottom = row >= topRows;
    boolean right  = col >= leftCols;

    if (!bottom && !right) return 1; // top-left
    if (!bottom &&  right) return 2; // top-right
    if ( bottom && !right) return 3; // bottom-left
